...
int main(void)
{
  ...
  // Set time on DS3231: 15:50:00, 3nd day of the week, 16th day of the month, April 2025
  // Set_Time(10, 04, 18, 3, 16, 4, 25);
...
void switchToControlMode(void)
{
    ...
    if (lastbutton == 0) {
    	...
    	if (direction == 0) {
    		// direction bằng 0 thì cho phép chiều hiện tại
    		uint32_t start_time  = HAL_GetTick();
    		lastbutton = 0;
    		...
    		// direction khác 0 thì chặn chiều hiện tại
    		lastbutton = 0;
    	    uint32_t start_time  = HAL_GetTick();
...
UNUSED(GPIO_Pin);
  ...
  {
    // DS3231 Alarm 1 triggered via INT/SQW
